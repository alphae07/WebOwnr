...
export default function ElegantBrand({ site }: { site: any }) {
  return (
    <div className="font-serif">
      <header className="bg-white shadow-md p-6">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <h1 className="text-3xl font-semibold text-gray-800">{site.businessName}</h1>
      ...
      <section className="bg-gray-100 py-28 text-center">
        <h2 className="text-4xl font-bold text-gray-900 mb-4">{site.tagline}</h2>
        <p className="max-w-2xl mx-auto text-gray-600">{site.about}</p>
      </section>
      ...
      <section id="services" className="py-20 bg-white">
        <div className="max-w-6xl mx-auto text-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
            {(site.services || ["Brand Identity", "Website Design", "Social Media Kit"]).map((service: string, i: number) => (
      ...
      <section id="contact" className="bg-gray-900 text-white py-16 text-center">
        ...
        <p>Email: {site.email || "hello@webownr.com"}</p>
        <p>Phone: {site.phone || "+234XXXXXXXXXX"}</p>
      </section>

      <footer className="bg-gray-800 text-white py-6 text-center">
        &copy; {new Date().getFullYear()} {site.businessName}. Made with elegance by WebOwnr.
      </footer>
    </div>
  );
}
